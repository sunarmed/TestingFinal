...
void ATPG::tdfault_sim_a_vector_int(const vector<int>& vec, int& num_of_current_detect){
  int i, nckt;
  fptr f;
  ...
  tdfault_sim_a_vector2_int(vec, num_of_current_detect);
}
...
void ATPG::tdfault_sim_a_vector2_int(const vector<int>& vec, int& num_of_current_detect) {
  wptr w,faulty_wire;
  /* array of 16 fptrs, which points to the 16 faults in a simulation packet  */
  fptr simulated_fault_list[num_of_pattern];
  fptr f;
  ...
  for(i = 0; i < cktin.size(); i++) {
    if(i == 0)
    cktin[i]->value = vec[cktin.size()];
    else  
    cktin[i]->value = vec[i-1];
  }
...
void ATPG::generate_tdfault_list(void) {
  int fault_num;
  wptr w;
  nptr n;
  fptr_s f;
